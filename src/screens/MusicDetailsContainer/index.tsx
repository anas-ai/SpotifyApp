...
const MusicDetailsScreen = ({ navigation, route }) => {
  ...
  const [playing, setPlaying] = useState(false);
  const [currentSong, setCurrentSong] = useState(selectedSong || songsList?.[0]);
  const [currentIndex, setCurrentIndex] = useState(songsList?.findIndex(song => song.id === selectedSong?.id) || 0);
  const [position, setPosition] = useState(0);
  const [duration, setDuration] = useState(1);
  const [isSliding, setIsSliding] = useState(false);
  const [showBottomPlayer, setShowBottomPlayer] = useState(selectedSong || songsList?.[0]);
  const [modalVisible, setModalVisible] = useState(false);
  ...
  // Handle song changes
  useEffect(() => {
    if (songsList?.[currentIndex]) {
      setCurrentSong(songsList[currentIndex]);
      loadSong(songsList[currentIndex]);
    }
  }, [currentIndex, songsList, loadSong]);

  // Initialize player on mount
  useEffect(() => {
    setupPlayer();
    return () => TrackPlayer.reset(); // Cleanup on unmount
  }, [setupPlayer]);

  // Track player events (e.g., song end)
  useTrackPlayerEvents([Event.PlaybackTrackChanged], async event => {
    if (event.nextTrack === null && currentIndex < songsList.length - 1) {
      playNextSong();
    }
  });
  ...
  const playNextSong = useCallback(() => {
    setCurrentIndex(prev => (prev < songsList.length - 1 ? prev + 1 : 0));
  }, [songsList]);

  const playPreviousSong = useCallback(() => {
    setCurrentIndex(prev => (prev > 0 ? prev - 1 : songsList.length - 1));
  }, [songsList]);
  ...
  const restSongs = songsList?.filter(item => item?.id !== currentSong?.id) || [];
  ...
  return (
    <LinearGradient
      ...
      <View>
        ...
        />
        <View style={styles.ArtisImageContainer}>
          <Image source={{ uri: currentSong?.artwork }} style={styles.ArtisImage} />
        </View>
        <ResponsiveText
          title={currentSong?.title || 'Unknown Title'}
          fontColor={colors.gray}
      ...
      <ResponsiveText
        title={`- ${formatTime(Math.max(0, duration - position))}`}
        fontColor={colors.gray}
        fontSize={13}
        fontWeight="500"
        fontStyle={{ marginTop: 10 }}
      ...
      <View style={styles.ControlsStyleContainer}>
        ...
        <View style={styles.shuffleContainer}>
          <TouchableOpacity style={{ marginRight: 10 }} onPress={playNextSong}>
            <ShuffleIcon name="shuffle" size={30} color={colors.white} />
          </TouchableOpacity>
          <View style={styles.PlayPauseContainer}>
            {playing ? (
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            onPress={() => {
              const index = songsList.findIndex(song => song.id === item.id);
      ...
      {showBottomPlayer && (
        <View
          ...
          <Slider
            ...
            onSlidingComplete={handleSlidingComplete}
            minimumTrackTintColor={colors.Neon_Pink}
      ...
      />
      <RecommenDationsModel
        isVisible={recommendationsModalVisible}
        setIsVisible={setRecommendationsModalVisible}
      />
    </LinearGradient>
  );
};
